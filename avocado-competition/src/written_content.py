...
ABOUT_THE_SIMULATION = {
    ...
    'Sim Setup': [
        ...
        neutral voters and/or voters who really like/dislike guacamole and thus tend to score higher/lower."""        
    ], 
    'Guac Config': [
    ...
    'How Voting Works': [
        ...
        For voters who really like/dislike guacamole, votes are sampled from a normal distribution centered at TS+2 for voters who like and TS-2 for voters who dislike.
...
STORY = {
    ...
    "simulation_1_deep_dive": [
        ...
        and percentages of people who really like guacs (pct_ppl_like) from 0% to 50% (top to bottom). 
    ...
    "simulation_2": [
        ...
        The winning guacamole will be determined with a condorcet method (more appropriate for such configuration) as the one who wins a 
    ...
    "simulation_2_conclusion": [
        ...
        winner a bit more difficult, expecially as the number of guacamoles for each voter is dialed down. 
        ...
        """Below, we explore the parameter space in standard deviations (std) and number of townpeople 
        for the configuration where we expect to loose the true winner more easily: A Lot Of Contenders."""
    ], 
    "simulation_2_deep_dive": [
        ...
        For simplicity, each people character accounts for about 1/3 of voters. 
        We consider std ranging from 2 to 4 (left to right) and number of townpeople from 100 to 300 (top to bottom). 
        As before, increasing std (left to right) decreases the fraction of time
        the true winner is recovered. Instead, increasing the number of townpeople has the opposite effect, 
        as it increases the fraction of time the true winner is recovered.
        """
    ], 
    "Conclusions": [
        """Whether we’re assessing which coworker makes the best chili or which movie should take home 
        the academy award, these simulated contests show that we don’t need every judge in our contest 
        to assess every single entry for the competition to have a fair outcome. 
        Limiting how many entries each judge gets to assess creates an opportunity 
        to invite more participants into the contest. Also, with less entries to judge, 
        this feature allows each participant to be more discerning than they perhaps could be otherwise. 
        """
    ]
